...
const parseAiSuggestion = (text) => {
  ...
  const videoHeaderIndex = lines.findIndex((line) =>
  ...
  const weakLine = beforeVideo.find((line) =>
    line.toLowerCase().startsWith("weak sections:"),
  );
  const weakSections = weakLine
    ? weakLine
        .slice(weakLine.indexOf(":") + 1)
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean)
    : [];
  ...
  const technicalLines = afterVideo.filter(
    (line) =>
      !line.includes("[Video Lecture Quality Review]") &&
      !line.includes("----------------------------"),
  );
...
const isSuggestionUnavailable = (suggestion) => {
  ...
  return (
    !value ||
    value.includes("suggestion unavailable") ||
    value.includes("no suggestion available")
  );
};
...
const severityBadgeClass = (severity = "") => {
  ...
  if (value.includes("high"))
    return "bg-soft-danger text-danger border border-danger border-opacity-25";
  if (value.includes("medium"))
    return "bg-soft-warning text-warning border border-warning border-opacity-25";
  if (value.includes("low"))
    return "bg-soft-success text-success border border-success border-opacity-25";
...
const AiSuggestionPreview = ({
  ...
  const recommendation =
    technical.recommendation || mainPoints[0] || suggestion;
        ...
        {weakSections.length > 0 && (
          <div className="d-flex flex-wrap gap-1 mb-2">
            {weakSections.slice(0, 4).map((section) => (
              <span
                key={section}
                className="badge bg-soft-warning text-warning border border-warning border-opacity-25"
              >
                {section}
              </span>
            ))}
...
const ViewLectureFeedback = () => {
    ...
    const fetchFeedbacks = async () => {
      try {
        ...
        const res = await axios.get(
          `${BASE_URL}/teacher-guide-feedbacks/lectureId/${lectureId}`,
          {
        ...
        Swal.fire(
          "Error",
          "Failed to load feedbacks for this lecture.",
          "error",
        );
      } finally {
        setLoading(false);
  ...
  const generateSuggestionFor = async (fb) => {
    ...
    const langConstraint =
      "*** MANDATORY: RESPOND IN ENGLISH ONLY. NO CHINESE CHARACTERS ALLOWED. ***";

    const feedbackPayload = {
      model_type: "feedback",
      feedback_data: {
        instruction:
          "RESPOND IN ENGLISH ONLY. Give one specific recommendation using the lesson topic, grade, and student feedback. Avoid generic advice.",
                    ...
                    <div className="row mb-5 justify-content-center">
                      <div className="col-md-8 col-lg-6">
                        <div className="p-4 border rounded shadow-sm bg-white">
                          <h6 className="text-center fw-bold mb-4 text-primary">
                            Feedback Distribution Overview
                          </h6>
                          <ReactApexChart
                            ...
                            <tr key={fb._id}>
                              <td>
                                <span className="fw-medium">
                                  {fb?.studentId?.username ||
                                    fb?.studentId?.email ||
                                    "Unknown"}
                                </span>
                                <div className="text-muted small">
                                  Age: {fb?.studentId?.age || "-"}
                                </div>
                              </td>
                              <td>
                                <div
                                  ...
                                  onClick={() =>
                                    handleNavigateToDetailedView(fb)
                                  }
                                >
                                  {fb?.studentFeedback || (
