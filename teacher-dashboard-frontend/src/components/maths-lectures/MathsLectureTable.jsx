...
const MathsLectureTable = ({ title }) => {
  const navigate = useNavigate();

  // lectures list from /maths/video-lectures
  ...
  const fetchLectures = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/maths/video-lectures`, {
  ...
  const handleDeleteLecture = async (lectureId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const token = getToken();
        await axios.delete(`${BASE_URL}/maths/video-lectures/${lectureId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleActionClick = (action, lecture) => {
    if (action === 'Delete') {
      ...
      navigate(`/admin/maths-lectures/edit/${lecture._id}`);
    }
  };
  ...
  const getDropdownItems = (lecture) => [
    {
      icon: <FiExternalLink />,
      label: 'View Feedback',
      onClick: () => navigate(`/admin/maths-lectures/feedback/view/${lecture._id}`)
    },
    { type: 'divider' },
    {
      icon: <FiEdit3 />,
        ...
        <div className="card-body custom-card-action p-0">
          <div className="table-responsive" style={{ minHeight: "350px" }}>
            <table className="table table-hover mb-0" style={{ tableLayout: 'fixed', width: '100%' }}>
              <thead>
                <tr>
                  <th style={{ width: '24%' }}>Lecture Title</th>
                  <th style={{ width: '11%' }}>Difficulty</th>
                  <th style={{ width: '20%' }}>Teacher Guide</th>
                  <th style={{ width: '8%' }}>Video</th>
                  <th style={{ width: '9%' }}>Materials</th>
                  <th style={{ width: '14%' }}>Created By</th>
                  <th style={{ width: '10%' }}>Created At</th>
                  <th className="text-end" style={{ width: '9%', whiteSpace: 'nowrap' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {paginatedLectures.length === 0 ? (
                  ...
                  paginatedLectures.map((lec) => {
                    ...
                    const createdAtDate = lec?.createdAt ? new Date(lec.createdAt) : null;
                    const createdDate = createdAtDate ? createdAtDate.toLocaleDateString() : '-';
                    const createdTime = createdAtDate ? createdAtDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
                    ...
                    return (
                      <tr key={lec._id}>
                        ...
                        <td
                          style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                          title={createdAtDate ? createdAtDate.toLocaleString() : '-'}
                        >
                          <div className="fw-semibold">{createdDate}</div>
                          {createdTime && <small className="text-muted">{createdTime}</small>}
                        </td>
...
export default MathsLectureTable;
