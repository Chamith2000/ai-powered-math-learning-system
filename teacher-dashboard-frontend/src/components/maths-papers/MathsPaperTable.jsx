...
const MathsPaperTable = ({ title }) => {
  ...
  const fetchPapers = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/maths/papers`, {
  ...
  const handleDeletePaper = async (paperId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const token = getToken();
        await axios.delete(`${BASE_URL}/maths/papers/${paperId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleActionClick = (action, paper) => {
    if (action === 'Delete') {
      ...
      navigate(`/admin/maths-papers/edit/${paper._id}`);
    } else if (action === 'View') { // <-- NEW
      navigate(`/admin/maths-papers/view/${paper._id}`);
    }
  };
...
export default MathsPaperTable;
