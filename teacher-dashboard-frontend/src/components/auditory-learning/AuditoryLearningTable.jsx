...
const AuditoryLearningTable = ({ title }) => {
  ...
  const fetchAuditory = async () => {
    try {
      ...
      const res = await axios.get(`${BASE_URL}/auditory/learning`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = Array.isArray(res.data) ? res.data : [];
      setItems(data);
    } catch (err) {
      console.error('Failed to load auditory learning items', err);
      Swal.fire('Error', 'Failed to load auditory learning items.', 'error');
    }
  };
  ...
  const handleDeletes = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const token = getToken();
        await axios.delete(`${BASE_URL}/auditory/learning/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        Swal.fire('Deleted!', 'Auditory learning item has been deleted.', 'success');
        fetchAuditory();
      } catch (err) {
        Swal.fire('Error!', 'Failed to delete auditory learning item.', 'error');
      }
    }
  };
  ...
  const handleActionClick = (action, row) => {
    if (action === 'Delete') {
      ...
      navigate(`/admin/auditory-learning/edit/${row._id}`);
    } else if (action === 'View') {
      navigate(`/admin/auditory-learning/view/${row._id}`);
    }
  };
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-4">
                      No auditory learning items found.
                    </td>
                  </tr>
                ) : (
                  paginated.map((row) => {
...
export default AuditoryLearningTable;
