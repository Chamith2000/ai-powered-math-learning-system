...
const ViewVisualLearningForm = ({ title }) => {
  ...
  // ---- load visual + questions (NOTE: visual endpoints)
  useEffect(() => {
    const loadVisual = async () => {
      try {
        ...
        setVisual(res?.data || null);
      } catch (err) {
        console.error('Failed to load visual item', err);
        Swal.fire('Error', 'Failed to load visual learning item.', 'error');
      } finally {
    ...
    const loadQuestions = async () => {
      try {
        ...
        Swal.fire('Error', 'Failed to load questions for this visual.', 'error');
      } finally {
  ...
  const tgTitle = useMemo(() => {
    if (!visual) return '—';
    const tg =
      typeof visual?.teacherGuideId === 'string' ? null : visual?.teacherGuideId;
    return tg?.coureInfo || '—';
  }, [visual]);

  const creator = useMemo(() => {
    if (!visual) return {};
    return visual?.createby || visual?.createBy || {};
  }, [visual]);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      Swal.fire('Error', 'Failed to analyze this visual.', 'error');
    } finally {
      setAnalyzing(false);
    }
  };
      ...
      <div className={`card stretch stretch-full ${isExpanded ? 'card-expand' : ''} ${refreshKey ? 'card-loading' : ''}`}>
        ...
        <div className="card-body">
          ...
          <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            <div className="d-flex align-items-center gap-2">
              ...
              <button
                ...
                onClick={runAnalysis}
                disabled={analyzing || loadingVisual || loadingQuestions}
              >
          ...
          <div className="row g-3 mb-4">
            ...
            <div className="col-md-4">
              <div className="p-3 border rounded h-100">
                ...
                <div className="small">Created: {fmt(visual?.createdAt)}</div>
                <div className="small">Updated: {fmt(visual?.updatedAt)}</div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {loadingVisual || loadingQuestions ? (
              <div className="text-muted">Loading…</div>
            ) : questions.length === 0 ? (
              <div className="text-muted">No questions available for this visual.</div>
            ) : (
              <ol className="ps-3">
                {questions.map((q, idx) => {
                  const qid = q._id || `Q${String(idx + 1).padStart(2, '0')}`;
...
export default ViewVisualLearningForm;
