...
const TeacherGuideTable = ({ title }) => {
  ...
  const getDropdownItems = (guideId) => [
    ...
    { type: 'divider' },
    { icon: <FiTrash2 />, label: 'Delete', onClick: () => handleActionClick('Delete', guideId) },
  ];
            ...
            <table className="table table-hover mb-0">
              ...
              <tbody>
                {paginatedGuides.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-4">No teacher guides found.</td>
                  </tr>
                ) : (
                  paginatedGuides.map((g) => (
                    <tr key={g._id}>
                      ...
                      <td style={{ minWidth: '120px' }}>
                        {typeof g.studytime === 'number' ? g.studytime : (g.studytime ?? '-') }
                      </td>
