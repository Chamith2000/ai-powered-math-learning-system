...
const PaperDetails = () => {
  ...
  // timing – measure time from mount to submit (in seconds)
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      await apiClient.post("/api/teacher-guide-feedbacks", {
        ...
        studytime: secondsSpent,  // time spent (seconds) before submitting
      });

      Swal.fire({
        icon: "success",
        title: "Thanks for your feedback!",
        text: `Attempted ${attempted}/${questions.length} questions (${attemptedPct}%). Time spent: ${secondsSpent}s.`,
        timer: 1800,
            ...
            <div className="paper-content">
              ...
              <div className="text-center mt-4">
                ...
                {isSubmitted && (
                  <div className="mt-3">
                    ...
                    <div className="text-muted">Time spent: {secondsSpent}s</div>
                  </div>
                )}
              </div>
              ...
              {isSubmitted && (
                <div className="card mt-4 p-3">
                  <h5>
                    Leave Feedback{" "}
                    {paper.teacherGuideId?.coureInfo ? `for: ${paper.teacherGuideId.coureInfo}` : ""}
                  </h5>
                  {!paper.teacherGuideId?._id && (
                    <div className="alert alert-warning mb-3">
                      This paper has no attached teacher guide, so feedback can’t be submitted.
                    </div>
                  )}
              ...
              {/* Previous feedbacks (optional display) */}
            </div>
          ) : (
            <p>Paper not found.</p>
          )}
