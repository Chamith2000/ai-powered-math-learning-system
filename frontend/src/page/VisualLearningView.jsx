...
const VisualLearningView = () => {
  ...
  // avoid double counting user visual stats in a single page session
  ...
  const fetchQAs = async () => {
    try {
      ...
      const initial = {};
      shuffled.forEach(q => { initial[q._id] = ""; });
      setAnswers(initial);
      setQaResults({});
    } catch (err) {
  ...
  // --- Update the User's visual stats (/users/:id) ---
  // Increments VisualLearningCount by 1 and adds 'percent' to VisualLearningTotalMarks.
  // Only called when the lecture is NOT completed. Guarded to run once per page session.
  const updateUserVisualStats = async (percent) => {
    if (!userId) return;
    try {
      // read current user values
      const { data: user } = await apiClient.get(`api/users/${userId}`);
      const currCount = Number(user?.VisualLearningCount ?? 0);
      const currTotal = Number(user?.VisualLearningTotalMarks ?? 0);

      const payload = {
        VisualLearningCount: currCount + 1,
        VisualLearningTotalMarks: currTotal + Number(percent || 0),
      };

      await apiClient.put(`api/users/${userId}`, payload);
      userStatsUpdatedRef.current = true;
    } catch (e) {
      console.error("Failed to update user visual stats:", e);
    }
  };
  ...
  const handleCheckAnswers = async () => {
    ...
    try {
      ...
      const passed = percent > 50; // strictly "more than 75%"
      ...
      // 1) Update the user's Visual stats if NOT completed (once per session)
      if (!isCompleted && !userStatsUpdatedRef.current) {
        await updateUserVisualStats(percent);
      }

      // 2) Mark as completed if passed and not already marked
      if (passed && !isCompleted && !completionMarkedRef.current) {
        await markThisLectureCompleted();
      }

      // 3) Update learning-type stats (count + percentage points)
