...
const ReadAndWriteLearningDetails = () => {
  ...
  // avoid double user-stat updates in one page session
  ...
  const fetchPaperAndQuestions = async () => {
    ...
    try {
      ...
      const qsRaw = Array.isArray(qRes.data?.items) ? qRes.data.items : [];
      const qs = shuffle(qsRaw); // 🔀 shuffle on load
      setQuestions(qs);
  ...
  // --- NEW: update User's read/write stats (/users/:id) ---
  // Increments ReadWriteLearningCount by 1 and adds attempt % to ReadWriteLearningTotal.
  // Only if NOT completed, and only once per page session.
  const updateUserReadWriteStats = async (percentInt) => {
    if (!userId) return;
    try {
      const { data: user } = await apiClient.get(`api/users/${userId}`);
      const currCount = Number(user?.ReadWriteLearningCount ?? 0);
      const currTotal = Number(user?.ReadWriteLearningTotal ?? 0);

      const payload = {
        ReadWriteLearningCount: currCount + 1,
        ReadWriteLearningTotal: currTotal + Number(percentInt || 0),
      };

      await apiClient.put(`api/users/${userId}`, payload);
      userStatsUpdatedRef.current = true;
    } catch (e) {
      console.error("Failed to update user read/write stats:", e);
    }
  };
  ...
  const submitAnswers = async () => {
    ...
    // NEW: update User's read/write fields if NOT completed (once per session)
    if (!isCompleted && !userStatsUpdatedRef.current) {
      await updateUserReadWriteStats(percentInt);
    }

    // update read/write learning progress (learning-type collection)
