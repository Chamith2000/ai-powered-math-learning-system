...
RESULT_STDERR = _stderr.getvalue()
  ...
  // ---- NEW: update User's kinesthetic stats (/users/:id) ----
  // Increments KinestheticLearningCount by 1 and adds attempt % to KinestheticLearningTotal.
  // Only if NOT completed, and only once per page session.
  const updateUserKinestheticStats = async (attemptPercent) => {
    if (!userId) return;
    try {
      const { data: user } = await apiClient.get(`api/users/${userId}`);
      const currCount = Number(user?.KinestheticLearningCount ?? 0);
      const currTotal = Number(user?.KinestheticLearningTotal ?? 0);

      const payload = {
        KinestheticLearningCount: currCount + 1,
        KinestheticLearningTotal: currTotal + Math.round(Number(attemptPercent || 0)),
      };

      await apiClient.put(`api/users/${userId}`, payload);
      userStatsUpdatedRef.current = true;
    } catch (e) {
      console.error("Failed to update user kinesthetic stats:", e);
    }
  };
  ...
  const submitAnswer = async () => {
    ...
    // NEW: update User's kinesthetic fields if NOT completed (once per session)
    if (!isCompleted && !userStatsUpdatedRef.current) {
      await updateUserKinestheticStats(attemptPercent);
    }

    // update kinesthetic learning-type stats
